...
export default function LandingPrices() {
    ...
    const fetchCouponData = async () => {
        try {
            const couponData = await getCouponData({ planId: PaymentPlanId.Yearly }); // Since plans share same coupon, doesnt matter that only pass in yearly plan
            if (couponData.max_redemptions && couponData.times_redeemed) {
                setRedemptionsLeft(couponData.max_redemptions - couponData.times_redeemed);
            }
        } catch (error) {
