...
const HowItWorksComponent: React.FC<HowItWorksProps> = ({ onCompletion }) => {
  ...
  return (
    <div ref={componentRef} className="md:py-30 py-10 text-center text-white">
      ...
      <div className="flex flex-col md:flex-row justify-between items-stretch gap-5 md:gap-10 w-full px-60">
        <div ref={el => boxRefs.current[0] = el} className="fill-container flex flex-col justify-start items-center mx-auto bg-translucent text-white p-5 md:pt-30 rounded-3xl border border-white md:w-1/3" style={{ flex: 1 }}>
          ...
          <p className="lg:text-lg md:text-md text-sm text-left text-content">Pick a template and customize it exactly to your liking.</p>
        </div>
        <div ref={el => boxRefs.current[1] = el} className="fill-container flex flex-col justify-start items-center mx-auto bg-translucent text-white p-5 md:pt-30 rounded-3xl border border-white md:w-1/3" style={{ flex: 1 }}>
          <div className="fill-animation"></div>
          <h3 className="md:text-3xl text-lg text-left font-semibold text-content md:mb-4">Share Your Giveaway</h3>
          <p className="lg:text-lg md:text-md text-sm text-left text-content">Post your giveaway on your website and social media pages and immediately start gaining new leads and followers.</p>
        </div>
        <div ref={el => boxRefs.current[2] = el} className="fill-container flex flex-col justify-start items-center mx-auto bg-translucent text-white p-5 md:pt-30 rounded-3xl border border-white md:w-1/3" style={{ flex: 1 }}>
          <div className="fill-animation"></div>
          <h3 className="md:text-3xl text-lg text-left font-semibold text-content md:mb-4">Analyze Your Results</h3>
          <p className="lg:text-lg md:text-md text-sm text-left text-content">Raffle Leader's AI tools help you pick a winner and sift through your new leads.</p>
        </div>
      </div>
    </div>
  );
